...
# - Prevent legacy MICRO/LIVE exposure from reappearing.
# - Provide a safe baseline enforcement endpoint.
# - Whitelist MICRO/LIVE only when explicitly allowed.
#
# Important:
# By default no MICRO/LIVE strategy is whitelisted. This is intentional while
# market gate is CAUTIOUS and directional regime is BEAR/SHORT.
...
def v9_3_3_allowed_micro_keys() -> set[str]:
    raw = V933_MICRO_LIVE_WHITELIST
    if not raw:
        return set()
    return {x.strip() for x in raw.split(",") if x.strip()}


def v9_3_3_iter_strategy_sides(state: Dict[str, Any]) -> list[Dict[str, Any]]:
    rows: list[Dict[str, Any]] = []
    for strategy_name, strategy_cfg in (state.get("strategies") or {}).items():
        ...
        for symbol, symbol_cfg in (strategy_cfg.get("symbols") or {}).items():
            if not isinstance(symbol_cfg, dict):
                continue
            for side, side_cfg in symbol_cfg.items():
                if not isinstance(side_cfg, dict):
                    continue
                rows.append(
                    {
                        ...
                        "side": str(side).upper(),
                        "mode": str(side_cfg.get("mode", "OFF")).upper(),
                        "risk_pct": v8_float(side_cfg.get("risk_pct"), 0.0),
                        "key": v9_3_3_key(strategy_name, symbol, side),
                        "config": side_cfg,
                    }
                )
    return rows
...
def v9_3_3_active_micro_live_items(state: Optional[Dict[str, Any]] = None) -> list[Dict[str, Any]]:
    state = state or load_state()
    return [x for x in v9_3_3_iter_strategy_sides(state) if x.get("mode") in {"MICRO", "LIVE"} and x.get("strategy_enabled") is not False]


def v9_3_3_baseline_expected_map() -> Dict[str, Dict[str, Any]]:
    expected = {}
    for item in v9_3_3_iter_strategy_sides(V933_SAFE_BASELINE_STATE):
        expected[item["key"]] = {
            "mode": item["mode"],
...
def v9_3_3_compare_to_safe_baseline(state: Dict[str, Any]) -> Dict[str, Any]:
    expected = v9_3_3_baseline_expected_map()
    actual_items = v9_3_3_iter_strategy_sides(state)
    actual = {x["key"]: x for x in actual_items}
    ...
    for key, exp in expected.items():
        ...
        if act is None:
            missing.append(exp)
            continue
        if act.get("mode") != exp.get("mode") or abs(v8_float(act.get("risk_pct"), 0.0) - v8_float(exp.get("risk_pct"), 0.0)) > 1e-9:
            mismatches.append(
                {
                    "key": key,
                    "expected": exp,
    ...
    # Anything not in baseline but active PAPER/MICRO/LIVE is not automatically wrong,
    # but it must be visible.
    for key, act in actual.items():
        if key not in expected and act.get("mode") != "OFF":
            unexpected_active.append(
                {
                    "key": key,
                    "strategy": act.get("strategy"),
                    "symbol": act.get("symbol"),
...
def v9_3_3_strategy_state_guard(notify: bool = False) -> Dict[str, Any]:
    if not V933_STRATEGY_STATE_GUARD_ENABLED:
        return {"ok": False, "reason": "V933_STRATEGY_STATE_GUARD_DISABLED"}

    state = load_state()
    allowed = v9_3_3_allowed_micro_keys()
    active_micro = v9_3_3_active_micro_live_items(state)
    baseline = v9_3_3_compare_to_safe_baseline(state)
    ...
    for item in active_micro:
        ...
        if key not in allowed:
            reasons.append(f"UNWHITELISTED_{item.get('mode')}_{key}")
            actions.append("ENFORCE_SAFE_BASELINE_OR_DISABLE_UNWHITELISTED_MICRO")

    gate = {}
    regime = {}
    try:
        gate = v9_market_regime_gate(days=30, limit=PAPER_OUTCOME_MAX_EVENTS)
    except Exception as exc:
        gate = {"ok": False, "error": str(exc)}
        reasons.append(f"MARKET_GATE_UNAVAILABLE:{exc}")
    ...
    gate_level = str(gate.get("gate_level") or "UNKNOWN").upper()
    directional_regime = str(regime.get("regime") or "UNKNOWN").upper()

    if V933_BLOCK_LONG_MICRO_WHEN_GATE_NOT_NORMAL and gate_level not in {"NORMAL"}:
        for item in active_micro:
            if item.get("side") == "LONG":
                reasons.append(f"LONG_MICRO_BLOCKED_BY_GATE_{gate_level}_{item.get('key')}")
                actions.append("DOWNGRADE_LONG_MICRO_TO_PAPER_OR_OFF")

    if V933_BLOCK_LONG_MICRO_WHEN_NOT_BULL and directional_regime not in {"BULL", "TRANSITION"}:
        for item in active_micro:
            if item.get("side") == "LONG":
                reasons.append(f"LONG_MICRO_BLOCKED_BY_REGIME_{directional_regime}_{item.get('key')}")
                actions.append("DOWNGRADE_LONG_MICRO_TO_PAPER_OR_OFF")

    if baseline["mismatch_count"] > 0:
        reasons.append(f"SAFE_BASELINE_MISMATCHES_{baseline['mismatch_count']}")
        actions.append("ENFORCE_SAFE_BASELINE")

    # Missing keys are less critical, but make them visible.
    if baseline["missing_count"] > 0:
        reasons.append(f"SAFE_BASELINE_MISSING_KEYS_{baseline['missing_count']}")
        actions.append("ENFORCE_SAFE_BASELINE")

    if reasons:
        level = "CRITICAL" if active_micro or "ENFORCE_SAFE_BASELINE" in actions else "WATCH"
        recommendation = "ENFORCE_SAFE_BASELINE_BEFORE_TRADING_DECISIONS"
        decision_safe = False
    else:
        level = "OK"
        recommendation = "STRATEGY_STATE_MATCHES_SAFE_BASELINE"
        decision_safe = True
    ...
    result = {
        ...
        "active_micro_live_count": len(active_micro),
        "active_micro_live_items": active_micro,
        "micro_live_whitelist": sorted(list(allowed)),
        "baseline": baseline,
        "market_gate": {
            "ok": gate.get("ok"),
            "gate_level": gate.get("gate_level"),
        ...
        "guard_settings": {
            "auto_enforce_on_startup": V933_AUTO_ENFORCE_SAFE_BASELINE_ON_STARTUP,
            "notify_on_state_drift": V933_NOTIFY_ON_STATE_DRIFT,
            "block_long_micro_when_gate_not_normal": V933_BLOCK_LONG_MICRO_WHEN_GATE_NOT_NORMAL,
            "block_long_micro_when_not_bull": V933_BLOCK_LONG_MICRO_WHEN_NOT_BULL,
        },
    }
    ...
    if should_notify:
        try:
            safe_notify_event(
                f"🧯 Strategy State Guard: {level}",
                f"{recommendation}\nReasons: {', '.join(result['reasons']) if result['reasons'] else 'none'}\nActive MICRO/LIVE: {len(active_micro)}",
                important=level in {"WATCH", "CRITICAL"},
            )
            result["telegram_notified"] = True
        except Exception as exc:
    ...
    write_json_file(V933_STRATEGY_GUARD_STATE_FILE, result)
    return result


def v9_3_3_enforce_safe_baseline(reason: str = "v9_3_3_safe_baseline_enforce") -> Dict[str, Any]:
    require_strategy_admin()
    before = load_state()
    before_guard = v9_3_3_strategy_state_guard(notify=False)
    save_state(V933_SAFE_BASELINE_STATE)
    after = load_state()
    after_guard = v9_3_3_strategy_state_guard(notify=False)

    write_system_log(
        action="strategy_state_safe_baseline_enforced",
        symbol="SYSTEM",
        side="SYSTEM",
        decision="SAFE_BASELINE_ENFORCED",
        reason=reason,
        status="logged",
        extra={
            "before_active_micro_live_count": before_guard.get("active_micro_live_count"),
            "after_active_micro_live_count": after_guard.get("active_micro_live_count"),
            "before_guard_level": before_guard.get("level"),
            "after_guard_level": after_guard.get("level"),
        },
    )
...
@app.get("/v9_3_3_strategy_state_guard")
def v9_3_3_strategy_state_guard_endpoint(secret: str, notify: bool = False):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")
    return v9_3_3_strategy_state_guard(notify=notify)


@app.post("/v9_3_3_enforce_safe_baseline")
async def v9_3_3_enforce_safe_baseline_endpoint(request: Request):
    body = await request.json()
    verify_secret(request, body)
    confirm = str(body.get("confirm") or "").upper()
    if confirm != "ENFORCE_SAFE_BASELINE":
        raise HTTPException(400, "Set confirm='ENFORCE_SAFE_BASELINE' to enforce the safe baseline.")
    return v9_3_3_enforce_safe_baseline(reason=body.get("reason") or "manual_v9_3_3_enforce_safe_baseline")


@app.get("/v9_3_3_safe_baseline")
def v9_3_3_safe_baseline_endpoint(secret: str):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")
    return {"ok": True, "version": APP_FEATURE_LEVEL, "state": V933_SAFE_BASELINE_STATE}


@app.get("/v9_3_3_strategy_state_guard_dashboard", response_class=HTMLResponse)
def v9_3_3_strategy_state_guard_dashboard(secret: str, notify: bool = False):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")

    data = v9_3_3_strategy_state_guard(notify=notify)
    active_rows = "".join(
        [
            f"<tr><td>{h(x.get('strategy'))}</td><td>{h(x.get('symbol'))}</td><td>{h(x.get('side'))}</td><td>{h(x.get('mode'))}</td><td>{fmt_num(x.get('risk_pct'))}</td><td>{h(x.get('key'))}</td></tr>"
            for x in data.get("active_micro_live_items", [])
        ]
    )
    mismatch_rows = "".join(
        [
            f"<tr><td>{h(x.get('key'))}</td><td>{h(x.get('expected'))}</td><td>{h(x.get('actual'))}</td></tr>"
            for x in (data.get("baseline") or {}).get("mismatches", [])[:50]
        ]
    )
    ...
    return HTMLResponse(
        ...
        <body>
          ...
          <div class="card">
            <h2>Active MICRO/LIVE items</h2>
            <b>Count:</b> {data.get('active_micro_live_count')}
            <table>
              <tr><th>Strategy</th><th>Symbol</th><th>Side</th><th>Mode</th><th>Risk %</th><th>Key</th></tr>
              {active_rows}
            </table>
          </div>
          <div class="card">
            <h2>Safe baseline mismatches</h2>
            <b>Mismatch count:</b> {(data.get('baseline') or {}).get('mismatch_count')} |
            <b>Missing count:</b> {(data.get('baseline') or {}).get('missing_count')} |
            <b>Unexpected active:</b> {(data.get('baseline') or {}).get('unexpected_active_count')}
            <table>
              <tr><th>Key</th><th>Expected</th><th>Actual</th></tr>
              {mismatch_rows}
            </table>
          </div>
          <p>
            <a href="/v9_3_3_strategy_state_guard?secret={h(secret)}&notify=false">Guard JSON</a> ·
            <a href="/v9_3_3_safe_baseline?secret={h(secret)}">Safe baseline JSON</a> ·
            <a href="/strategy_state?secret={h(secret)}">Current strategy state</a>
          </p>
          <p><b>Enforce endpoint:</b> POST /v9_3_3_enforce_safe_baseline with JSON: {"secret":"...","confirm":"ENFORCE_SAFE_BASELINE"}</p>
        </body>
        </html>
        """
    )
...
@app.get("/v9_3_3_control_panel", response_class=HTMLResponse)
def v9_3_3_control_panel(secret: str):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")
...
# ============================================================
# v9.4.2/9.4.3 used raw Supabase trade_events. Those rows are often only
# LOGGED/ORDER_SENT and do not directly contain the evaluated outcome status/R.
# This hotfix uses the existing paper outcome evaluator:
# fetch_paper_events_for_outcome() -> evaluate_paper_trade()
# so it sees LOSS_SL / WIN_TP2 / PARTIAL_TP1_THEN_SL again.

V944_DAYS_SHORT = int(os.getenv("V944_DAYS_SHORT", "10"))
V944_DAYS_LONG = int(os.getenv("V944_DAYS_LONG", "30"))
V944_LIMIT = int(os.getenv("V944_LIMIT", "1000"))
V944_MIN_TRADES_FOR_MICRO_REVIEW = int(os.getenv("V944_MIN_TRADES_FOR_MICRO_REVIEW", "5"))
V944_MIN_TOTAL_R_FOR_MICRO_REVIEW = float(os.getenv("V944_MIN_TOTAL_R_FOR_MICRO_REVIEW", "2.0"))
V944_MIN_AVG_R_FOR_MICRO_REVIEW = float(os.getenv("V944_MIN_AVG_R_FOR_MICRO_REVIEW", "0.20"))
V944_DISABLE_AVG_R_THRESHOLD = float(os.getenv("V944_DISABLE_AVG_R_THRESHOLD", "-0.35"))
V944_DISABLE_TOTAL_R_THRESHOLD = float(os.getenv("V944_DISABLE_TOTAL_R_THRESHOLD", "-2.0"))


def v9_4_4_active_keys() -> Dict[str, Dict[str, Any]]:
    state = load_state()
    active = {}
    for item in v9_3_3_iter_strategy_sides(state):
        side = str(item.get("side") or "").upper().strip()
        mode = str(item.get("mode", "OFF")).upper().strip()
        if side not in {"LONG", "SHORT"}:
            continue
        if mode not in {"PAPER", "MICRO", "LIVE"}:
            continue
        key = v9_3_3_key(item.get("strategy"), item.get("symbol"), side)
        active[key] = {
            "strategy": item.get("strategy"),
            "symbol": normalize_symbol(item.get("symbol")),
            "side": side,
...
def v9_4_4_fetch_outcomes(days: int, limit: int) -> list[Dict[str, Any]]:
    events = fetch_paper_events_for_outcome(days=max(1, int(days)), limit=max(1, int(limit)))
    outcomes = []
    for event in events:
        try:
            outcomes.append(evaluate_paper_trade(event))
        except Exception as exc:
            # Keep the report robust; one malformed event must not break the dashboard.
            outcomes.append({
...
def v9_4_4_group_outcomes(outcomes: list[Dict[str, Any]]) -> Dict[str, list[Dict[str, Any]]]:
    grouped = {}
    for outcome in outcomes or []:
        key = v9_4_4_outcome_key(outcome)
        grouped.setdefault(key, []).append(outcome)
    return grouped


def v9_4_4_summary(outcomes: list[Dict[str, Any]]) -> Dict[str, Any]:
    total = 0
    closed = 0
    open_count = 0
    wins = 0
    ...
    for item in outcomes or []:
        ...
        r = to_float_or_none(item.get("r_multiple"))
        if r is None:
            continue

        closed += 1
        total_r += float(r)
        if status == "WIN_TP2" or float(r) > 0:
            wins += 1
        elif status == "LOSS_SL" or float(r) < 0:
            losses += 1
        elif status == "PARTIAL_TP1_THEN_SL" or float(r) == 0:
            partial += 1
...
def v9_4_4_decision(cfg: Dict[str, Any], s30: Dict[str, Any], s10: Dict[str, Any]) -> Dict[str, Any]:
    c30 = int(s30.get("closed_count") or 0)
    c10 = int(s10.get("closed_count") or 0)
    r30 = float(s30.get("total_r") or 0.0)
    r10 = float(s10.get("total_r") or 0.0)
    ...
    if c10 >= V944_MIN_TRADES_FOR_MICRO_REVIEW and r10 >= V944_MIN_TOTAL_R_FOR_MICRO_REVIEW and (a10 or 0) >= V944_MIN_AVG_R_FOR_MICRO_REVIEW:
        decision = "MICRO_REVIEW_READY"
        reasons.append(f"10D_STRONG_C{c10}_R{round(r10,3)}_AVG{a10}")
        actions.append("MANUAL_REVIEW_ONLY_CHECK_REGIME_FIRST")
    elif c10 >= 2 and ((a10 is not None and a10 <= V944_DISABLE_AVG_R_THRESHOLD) or r10 <= V944_DISABLE_TOTAL_R_THRESHOLD):
        decision = "DISABLE_PAPER"
        reasons.append(f"10D_WEAK_C{c10}_R{round(r10,3)}_AVG{a10}")
        actions.append("CONSIDER_OFF_TO_REDUCE_NOISE")
    elif c30 >= 5 and ((a30 is not None and a30 <= V944_DISABLE_AVG_R_THRESHOLD) or r30 <= V944_DISABLE_TOTAL_R_THRESHOLD):
        if c10 > 0 and (a10 is not None and a10 > 0):
            decision = "WATCHLIST"
            reasons.append("30D_WEAK_BUT_10D_IMPROVING")
            actions.append("KEEP_PAPER_NO_MICRO")
    ...
    return {"decision": decision, "reasons": reasons, "actions": actions}


def v9_4_4_outcome_active_performance_report(days_long: int = V944_DAYS_LONG, days_short: int = V944_DAYS_SHORT, limit: int = V944_LIMIT) -> Dict[str, Any]:
    active = v9_4_4_active_keys()
    outcomes30 = v9_4_4_fetch_outcomes(days_long, limit)
    outcomes10 = v9_4_4_fetch_outcomes(days_short, limit)
    g30 = v9_4_4_group_outcomes(outcomes30)
    g10 = v9_4_4_group_outcomes(outcomes10)

    active_reports = []
    for key, cfg in active.items():
        s30 = v9_4_4_summary(g30.get(key, []))
        s10 = v9_4_4_summary(g10.get(key, []))
        dec = v9_4_4_decision(cfg, s30, s10)
        active_reports.append({**cfg, "key": key, **dec, "summary_30d": s30, "summary_10d": s10})

    historical = []
    for key, outcomes in g30.items():
        if key in active:
            continue
        s30 = v9_4_4_summary(outcomes)
        s10 = v9_4_4_summary(g10.get(key, []))
        if s30.get("closed_count") or s10.get("closed_count"):
            parts = key.split("|")
            historical.append({
                "key": key,
                "strategy": parts[0] if len(parts) > 0 else "",
    ...
    active30 = v9_4_4_summary([x for x in outcomes30 if v9_4_4_outcome_key(x) in active])
    active10 = v9_4_4_summary([x for x in outcomes10 if v9_4_4_outcome_key(x) in active])
    off30 = v9_4_4_summary([x for x in outcomes30 if v9_4_4_outcome_key(x) not in active])
    off10 = v9_4_4_summary([x for x in outcomes10 if v9_4_4_outcome_key(x) not in active])
    ...
    if any(r["decision"] == "MICRO_REVIEW_READY" for r in active_reports):
        gate_recommendation = "MICRO_REVIEW_POSSIBLE_MANUAL_ONLY"
    elif any(r["decision"] == "DISABLE_PAPER" for r in active_reports):
        gate_recommendation = "REDUCE_OR_DISABLE_WEAK_ACTIVE_PAPER"
    elif active10.get("closed_count", 0) >= 3 and (active10.get("average_r_closed") or 0) > 0:
        gate_recommendation = "KEEP_PAPER_COLLECT_MORE"
    else:
        gate_recommendation = "MONITOR_ONLY_NO_MICRO"
    ...
    return {
        ...
        "historical_off_summary": {"30d": off30, "10d": off10},
        "thresholds": {
            "min_trades_for_micro_review": V944_MIN_TRADES_FOR_MICRO_REVIEW,
            "min_total_r_for_micro_review": V944_MIN_TOTAL_R_FOR_MICRO_REVIEW,
            "min_avg_r_for_micro_review": V944_MIN_AVG_R_FOR_MICRO_REVIEW,
            "disable_avg_r_threshold": V944_DISABLE_AVG_R_THRESHOLD,
            "disable_total_r_threshold": V944_DISABLE_TOTAL_R_THRESHOLD,
        },
        "source": "paper_outcome_evaluator",
        "note": "Outcome-based report. Decision support only; no automatic state/risk change.",
    }


@app.get("/v9_4_4_outcome_active_performance_report")
def v9_4_4_outcome_active_performance_report_endpoint(secret: str, days_long: int = V944_DAYS_LONG, days_short: int = V944_DAYS_SHORT, limit: int = V944_LIMIT):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")
    return v9_4_4_outcome_active_performance_report(days_long=days_long, days_short=days_short, limit=limit)


@app.get("/v9_4_4_outcome_active_performance_dashboard", response_class=HTMLResponse)
def v9_4_4_outcome_active_performance_dashboard(secret: str, days_long: int = V944_DAYS_LONG, days_short: int = V944_DAYS_SHORT, limit: int = V944_LIMIT):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")
    report = v9_4_4_outcome_active_performance_report(days_long=days_long, days_short=days_short, limit=limit)
    ...
    <html>
    <head>
      <title>v9.4.4 Outcome-Based Active Performance Gate</title>
      <style>
        body{{font-family:Arial;margin:20px;background:#f6f8fb}}
        .card{{background:white;border-radius:12px;padding:14px;margin-bottom:14px;box-shadow:0 1px 6px #d1d5db}}
        table{{border-collapse:collapse;width:100%;background:white;font-size:12px;margin-top:10px}}
    ...
    <body>
      <h1>v9.4.4 Outcome-Based Active Strategy Performance Gate</h1>
      <div class="card">
        <b>Gate recommendation:</b> {h(report.get('gate_recommendation'))}<br>
        <b>By decision:</b> {h(report.get('by_decision'))}<br>
        <b>Source:</b> {h(report.get('source'))}<br>
        <b>Active summary 10d:</b> {h(report.get('active_summary',{}).get('10d'))}<br>
        <b>Active summary 30d:</b> {h(report.get('active_summary',{}).get('30d'))}<br>
        <b>Historical/off summary 10d:</b> {h(report.get('historical_off_summary',{}).get('10d'))}<br>
        <b>Historical/off summary 30d:</b> {h(report.get('historical_off_summary',{}).get('30d'))}
      ...
      <p>
        <a href="/v9_4_4_outcome_active_performance_report?secret={h(secret)}&days_long={days_long}&days_short={days_short}&limit={limit}">JSON report</a> ·
        <a href="/paper_outcome_summary?secret={h(secret)}&days=10&limit=1000">10d outcome summary</a> ·
        <a href="/paper_outcome_summary?secret={h(secret)}&days=30&limit=1000">30d outcome summary</a>
      </p>
    </body>
    </html>
    """)
...
@app.get("/v9_4_4_hotfix_note")
def v9_4_4_hotfix_note(secret: str):
    if secret != SHARED_SECRET:
        raise HTTPException(401, "Unauthorized")
    return {
        "ok": True,
